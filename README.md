...
- 문서마다 다른 검증 규칙과 오류 구조를 유연하게 처리하기 위해 스키마 기반 라이브러리 대신 순수 함수로 구현했습니다. 검증 로직을 UI와 분리해 단위 테스트가 가능하도록 구성했고, 복잡한 이력서는 공통 검증 흐름을 재사용하면서 단순한 문서는 필요한 검증만 구현하도록 설계했습니다.
