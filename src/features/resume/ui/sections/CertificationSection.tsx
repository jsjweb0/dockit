            ...
            {list.map((c, idx) => (
                <FieldGroup key={c.id} className="rounded-lg border p-4">
                    <div className="mb-3 flex items-center justify-between">
                        <div className="font-medium">자격증 {idx + 1}</div>
                        <Button
                            type="button"
                            variant="ghost"
                            onClick={() => remove(c.id)}
                    ...
                    <div className="grid gap-3 sm:grid-cols-2">
                        <Field>
                            <FieldLabel htmlFor={`acquiredAt-${c.id}`} className="text-sm text-muted-foreground">취득일</FieldLabel>
                            <Input
                                type="date"
                                id={`acquiredAt-${c.id}`}
                                value={c.acquiredAt}
                        ...
                        <Field>
                            <FieldLabel htmlFor={`certification-name-${c.id}`} className="text-sm text-muted-foreground">자격증</FieldLabel>
                            <Input
                    ...
                    <Field>
                        <FieldLabel htmlFor={`issuer-${c.id}`} className="text-sm text-muted-foreground">발행처</FieldLabel>
                        <Input
                            id={`issuer-${c.id}`}
                            type="text"
                            value={c.issuer}
