...
export function ProjectsSection({ value, onChange }: Props) {
    ...
    const add = () => {
        onChange({
            ...
            projects: [
                ...list,
                { id: uid(), name: "", period: "", stack: "", description: "", link: "" },
            ],
        });
    };
    ...
    return (
        <FieldSet>
            ...
            <div className="flex items-center justify-between gap-3">
                ...
                <Button type="button" variant="outline" onClick={add} aria-label="프로젝트 추가">
                    + 프로젝트 추가
                </Button>
            </div>
            <FieldSeparator />
            ...
            {list.map((p, idx) => (
                <FieldGroup key={p.id} className="rounded-lg border p-4">
                    ...
                    <div className="grid gap-3 sm:grid-cols-2">
                        <Field>
                            <FieldLabel htmlFor={`name-${p.id}`} className="text-sm text-muted-foreground">프로젝트명</FieldLabel>
                            <Input
                                id={`name-${p.id}`}
                                type="text"
                                value={p.name}
                        ...
                        <Field>
                            <FieldLabel htmlFor={`period-${p.id}`} className="text-sm text-muted-foreground">기간(자유)</FieldLabel>
                            <Input
                                id={`period-${p.id}`}
                                type="text"
                                value={p.period}
                        ...
                        <Field>
                            <FieldLabel htmlFor={`stack-${p.id}`} className="text-sm text-muted-foreground">기술 스택(요약)</FieldLabel>
                            <Input
                                id={`stack-${p.id}`}
                                type="text"
                                value={p.stack}
                        ...
                        <Field>
                            <FieldLabel htmlFor={`link-${p.id}`} className="text-sm text-muted-foreground">링크(선택)</FieldLabel>
                            <Input
                                id={`link-${p.id}`}
                                type="url"
                                value={p.link ?? ""}
                    ...
                    <Field>
                        <FieldLabel htmlFor={`description-${p.id}`} className="text-sm text-muted-foreground">설명(역할/기여/성과)</FieldLabel>
                        <Textarea
                            id={`description-${p.id}`}
                            value={p.description}
